(function () {
    ...
    setTimeout(function () {
        ...
        .then(r => r.json())
        .then(data => { chatApiUrl = data.result && data.result.api_url; });
        ...
        function applySize() {
            ...
            panel.style.right        = mobile ? '0px' : '24px';
            panel.style.bottom       = mobile ? '0px' : '90px';
            panel.style.borderRadius = mobile ? '0px' : '12px';
        }

        window.addEventListener('resize', function () {
            if (panel.style.display === 'flex') applySize();
        });
        ...
        function openPanel() {
            ...
            applySize();
            input.focus();
            if (!msgBox.children.length) appendWelcome();
        }
        ...
        function applyBotStyles(div) {
            ...
            div.querySelectorAll('pre').forEach(pre => pre.style.cssText = 'background:#e8e8e8;padding:8px;border-radius:6px;overflow-x:auto;font-size:0.8rem;');
        ...
        function makeAvatar(html, isBot) {
            ...
            if (isBot) {
                el.style.cssText = 'width:42px;height:42px;display:flex;align-items:center;justify-content:center;flex-shrink:0;background:transparent;';
            } else {
                el.style.cssText = 'width:38px;height:38px;background:#e8501a;border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;overflow:hidden;';
            }
            el.innerHTML = html;
            return el;
        }
        ...
        function appendMsg(text, from) {
            ...
            ts.style.cssText     = 'font-size:0.7rem;color:#aaa;';
            ...
            if (from === 'bot') {
                ...
                row.style.cssText     = 'display:flex;align-items:flex-end;gap:10px;justify-content:flex-end;';
                ...
                ts.style.paddingRight = '4px';
                col.append(bubble, ts);
                row.append(col, makeAvatar(USER_AVATAR));
            }
        ...
        function parseChunk(line, rawText, followUps, botDiv) {
            ...
            try {
                ...
                if (Array.isArray(parsed.follow_up) && parsed.follow_up.length) followUps = parsed.follow_up;
            } catch (_) {}
        ...
        async function sendChat() {
            ...
            ts.style.cssText       = 'font-size:0.7rem;color:#aaa;padding-left:4px;';
            ts.textContent         = getTime();
            col.style.cssText      = 'display:flex;flex-direction:column;gap:3px;max-width:82%;';
            row.style.cssText      = 'display:flex;align-items:flex-start;gap:10px;';

            col.append(botDiv, ts);
            row.append(avatar, col);
            msgBox.append(row);
            msgBox.scrollTop = msgBox.scrollHeight;
